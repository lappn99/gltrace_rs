...
use libc::user;
use wrapper::{
    glhooker_gethook, glhooker_gethookname, glhooker_getoriginalfunction, glhooker_gethookuserdata, glhooker_init, GLHookerHookType, GLHookerRegisterHookDesc, HookHandle
};
use std::mem;
...
impl<'a, T> HookDesc<'a, T> {
    pub fn new(hook_type: HookType, source_name: &'a str, dest_func: *mut c_void) -> Self {
        HookDesc { hook_type: hook_type, source_func_name: source_name, dst_func: dest_func, userdata: None }
    }
...
impl<'a> Hook{
    ...
    pub fn get_target_function(&self) -> Result<*mut c_void> {
        let func = unsafe {
            glhooker_getoriginalfunction(self.internal_hook)
...
mod tests {
    ...
    pub fn test_load_one() -> Result<()> {
        GLHooker::init()?;
        gl_loader::init_gl();
        let hook: HookDesc<_> = HookDesc::<()>::new(
            HookType::Intercept,
            "glBindBuffer",
            hook as *mut c_void,
        );
    ...
    pub fn test_get_userdata() -> Result<()> {
        ...
        let hook: HookDesc<_> = HookDesc::<i32>::new(
            HookType::Intercept,
            "glBindBuffer",
            hook as *mut c_void,
